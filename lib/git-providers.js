...
class GitHubProvider {
  ...
  async _getRepoCommits(repoFullName, since, until) {
    ...
    // author 필터 없이 전체 커밋 가져오기 (git config email이 GitHub에 연결 안 된 경우 대비)
    const url = `${this.baseUrl}/repos/${repoFullName}/commits?since=${since}&until=${until}&per_page=100`;
    console.log(`GitHub API 호출: ${url}`);
    const res = await fetch(url, { headers: this.headers() });
    if (!res.ok) {
      console.warn(`GitHub API ${res.status}: ${repoFullName}`);
      return [];
    }
    const data = await res.json();
    ...
    const filtered = data.filter((c) => {
      ...
      if (authorLogin === loginLower) return true;

      // 2) GitHub 계정 미연결 (author: null): commit.author.email로 매칭
      const commitEmail = c.commit?.author?.email?.toLowerCase() || '';
      if (commitEmail && myEmails.has(commitEmail)) return true;

      // 3) noreply 이메일 매칭 (GitHub이 자동 생성하는 이메일)
      const noreplyEmail = `${login.toLowerCase()}@users.noreply.github.com`;
      if (commitEmail === noreplyEmail) return true;

      return false;
    });

    console.log(`GitHub ${repoFullName}: 전체 ${data.length}건 중 내 커밋 ${filtered.length}건 (login=${login}, emails=${Array.from(myEmails).join(',')})`);
