...
class GitHubProvider {
  constructor(token, username, extraEmails = []) {
    this.token = token;
    this.username = username;
    this.extraEmails = extraEmails; // Gitea 등 다른 설정에서 가져온 이메일
  ...
  /**
   * GitHub 계정에 등록된 이메일 목록 가져오기 (캐시)
   * git config email이 GitHub에 연결 안 된 커밋도 매칭하기 위해 사용
   */
  async _getMyEmails() {
    if (this._myEmails) return this._myEmails;
    const emails = new Set();

    // 1) 다른 설정에서 가져온 이메일 (Gitea git config email 등)
    for (const e of this.extraEmails) {
      if (e) emails.add(e.toLowerCase());
    }

    try {
      // 2) /user/emails → GitHub에 등록된 이메일
      const res = await fetch(`${this.baseUrl}/user/emails`, { headers: this.headers() });
      if (res.ok) {
      ...
      // 3) /user → 프로필 이메일
      const res2 = await fetch(`${this.baseUrl}/user`, { headers: this.headers() });
      if (res2.ok) {
        const user = await res2.json();
        if (user.email) emails.add(user.email.toLowerCase());
      }
    } catch (e) {
      console.warn('GitHub emails API 실패:', e);
    }
    this._myEmails = emails;
    console.log('GitHub 매칭 이메일:', Array.from(emails));
    return emails;
  }
  ...
  async getCommitsByDate(since, until, selectedRepos = [], localDate = '') {
    let repoNames;
  ...
  async _getActiveRepos(since) {
    ...
    const url = `${this.baseUrl}/users/${encodeURIComponent(login)}/events?per_page=100`;
    console.log(`GitHub events API 호출: ${url}`);
    const res = await fetch(url, { headers: this.headers() });
  ...
  /**
   * 레포의 모든 브랜치 이름 조회 (페이지네이션 포함)
   * GitHub /commits API는 sha 미지정 시 default branch만 반환하므로,
   * 모든 브랜치를 돌며 커밋을 수집해야 함.
   */
  async _getRepoBranches(repoFullName) {
    ...
    try {
      let page = 1;
      while (true) {
        const url = `${this.baseUrl}/repos/${repoFullName}/branches?per_page=100&page=${page}`;
        const res = await fetch(url, { headers: this.headers() });
        if (!res.ok) {
          console.warn(`GitHub 브랜치 API ${res.status}: ${repoFullName}`);
          break;
        }
        const data = await res.json();
        if (!Array.isArray(data) || data.length === 0) break;
        for (const b of data) if (b && b.name) branches.push(b.name);
        if (data.length < 100) break;
        page++;
        if (page > 20) break; // 최대 2000개 브랜치까지
      }
    } catch (e) {
      console.warn(`GitHub 브랜치 목록 조회 실패: ${repoFullName}`, e);
    }
    // 아무것도 못 가져오면 default branch 대용으로 main 시도
    return branches.length > 0 ? branches : ['main'];
  }

  /**
   * 특정 브랜치에서 since~until 범위 커밋 가져오기 (페이지네이션)
   */
  async _getBranchCommits(repoFullName, branch, since, until) {
    const commits = [];
    const branchParam = encodeURIComponent(branch);
    let page = 1;
    while (true) {
      const url = `${this.baseUrl}/repos/${repoFullName}/commits?sha=${branchParam}&since=${since}&until=${until}&per_page=100&page=${page}`;
      const res = await fetch(url, { headers: this.headers() });
      if (!res.ok) {
        // 빈 레포, 권한 부족 등은 조용히 스킵
        if (res.status !== 404 && res.status !== 409) {
          console.warn(`GitHub API ${res.status}: ${repoFullName}@${branch}`);
        }
        break;
      }
      const data = await res.json();
      ...
      if (data.length < 100) break;
      page++;
      if (page > 5) break; // 안전장치 (하루 500커밋 상한)
    }
    return commits;
  }
  ...
  async _getRepoCommits(repoFullName, since, until) {
    const login = await this._getLoginName();
    const myEmails = await this._getMyEmails();
    const loginLower = login.toLowerCase();
    const noreplyEmail = `${loginLower}@users.noreply.github.com`;

    const branches = await this._getRepoBranches(repoFullName);
    console.log(`GitHub ${repoFullName}: 브랜치 ${branches.length}개 스캔 (${branches.slice(0, 10).join(',')}${branches.length > 10 ? ',...' : ''})`);
    ...
    for (let bi = 0; bi < branches.length; bi += BRANCH_BATCH) {
      const batch = branches.slice(bi, bi + BRANCH_BATCH);
      const results = await Promise.all(
        batch.map((branch) =>
          this._getBranchCommits(repoFullName, branch, since, until).catch((e) => {
            console.warn(`GitHub ${repoFullName}@${branch} 실패:`, e);
            return [];
          })
        )
      );
      ...
      for (const commits of results) {
        ...
        for (const c of commits) {
          ...
          // 내 커밋 매칭
          // 1) GitHub 계정 연결된 경우: author.login 매칭
          const authorLogin = c.author?.login?.toLowerCase() || '';
          // 2) 이메일 매칭 (git config email이 GitHub에 연결 안 된 경우 대비)
          const commitEmail = c.commit?.author?.email?.toLowerCase() || '';

          const isMe =
            authorLogin === loginLower ||
            (commitEmail && myEmails.has(commitEmail)) ||
            commitEmail === noreplyEmail;
    ...
    console.log(`GitHub ${repoFullName}: ${branches.length}개 브랜치에서 총 ${totalFetched}건(dedup ${seenSha.size}건) 중 내 커밋 ${filtered.length}건 (login=${login}, emails=${Array.from(myEmails).join(',')})`);
...
class GiteaProvider {
  ...
  async getCommitsByDate(since, until, selectedRepos = [], localDate = '') {
    ...
    // localDate = 로컬 타임존 기준 날짜 (YYYY-MM-DD)
    // since는 UTC이므로 split('T')[0]하면 전날이 될 수 있음
    const targetDate = localDate || since.split('T')[0];
    ...
    for (const repo of reposToSearch) {
      try {
        const commits = await this._getRepoCommits(repo, targetDate);
  ...
  async _getRepoCommits(repo, targetDate) {
    // targetDate = 로컬 타임존 기준 날짜 (YYYY-MM-DD)
